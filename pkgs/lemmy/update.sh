...
if ("$latest_version" === "$current_version") {
  ...
  const new_cargo_sha256 = $(nix-build -E "(import <nixpkgs> {}).callPackage $directory/server.nix {}" 2>&1 | \
    tail -n 2 | \
    head -n 1 | \
    sd '\s+got:\s+' '')

  const new_offline_cache_sha256 = $(nix-build -E "with import <nixpkgs> {}; callPackage $directory/ui.nix { nodejs = nodejs_18; }" 2>&1 | \
    tail -n 2 | \
    head -n 1 | \
    sd '\s+got:\s+' '')
