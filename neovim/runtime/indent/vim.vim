...
setlocal indentkeys+==endif,=enddef,=endfu,=endfor,=endwh,=endtry,=},=else,=cat,=finall,=END,0\\,0=\"\\\ ,=end,=EOF
...
function GetVimIndentIntern()
  ...
  if has('syntax_items')
    ...
    let syn_prev = synIDattr(synID(lnum, 1, 1), "name")
    if syn_here =~ 'vimLuaHeredoc\|vimScriptHeredoc' && cur_text =~ '^\s*EOF$'
      return 0
