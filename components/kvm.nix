...
({cfg, config, pkgs, lib, ...}: with lib; {
  ...
  config = {
    ...
    #  sed -i "s/KVMKVMKVM\\0\\0\\0/$hypervisor_string_replacement/g" target/i386/kvm/kvm.c
    #  sed -i 's/"bochs"/"<WOOT>"/g' block/bochs.c
    #  sed -i 's/"BOCHS "/"ALASKA"/g' include/hw/acpi/aml-build.h
    #  sed -i 's/Microsoft Hv/$hypervisor_string_replacement/g' target/i386/kvm/kvm.c
    #  echo "Applied!"
    #  '';
    #});
    environment.systemPackages = with pkgs; [ OVMF config.virtualisation.libvirtd.qemu.package virt-manager ];
