...
({cfg, config, pkgs, lib, ...}: with lib; {
  ...
  config = {
    ...
    systemd.services.dhclient = {
      ...
      dhclient -6 -P --prefix-len-hint 56 -d enp1s0u1u3 -e PATH="$PATH" -lf /var/lib/dhclient/leases -sf ${pkgs.dhcp}/sbin/dhclient-script
      '';
    };
