...

{
  ...
  nix.settings.substituters = if config.networking.hostName == "desktop" then [] else [ "http://desktop:9999" ];
  nix.settings.trusted-substituters = if config.networking.hostName == "desktop" then [] else [ "http://desktop:9999" ];
