...
with import ../components; rec {
  components = efi en_us est extra { graalvm = true; } gui { autoLogin = true; } kde steam docker home { deviceScaleFactor = 2; } kvm glances flatpak mqtt wg prometheus anbox ibus apcupsd { timeout = 300; minutes = 15; batteryLevel = 50; prometheus = "10.100.0.2"; } tpm2 tailscale postgres mosh usbmuxd nixbuild;
